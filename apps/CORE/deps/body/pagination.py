...
class PaginationRequest(BaseRequestSchema):
    next_token: StrOrNone = Field(
        default=None, description="nextToken from previous result.", alias="nextToken", examples=[None]
    )
...
class Pagination:
    ...
    async def __call__(
        ...
        if not request.state.sorting:
            raise NotImplementedError("You can't use Pagination without `Sorting`.")
    ...
    def create_next_token(self, latest_object: ObjectsVar | None, objects_count: int) -> StrOrNone:
    ...
    def read_next_token(self, next_token: StrOrNone) -> StrOrNone:
        ...
        try:
            ...
            next_token: str = json.loads(next_token_bytes, cls=ExtendedJSONDecoder)
